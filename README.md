...
#### Optional
- PR_TTLE: Title for the autogenerated translations PR
- PR_BODY: 'Body for the autogenerated translations PR
- PR_LABELS: Labels for the autogenerated translatons PR
- LLM_MODEL: LLM model to use. Currently supports 'gpt-4' (Recommeneded) and 'gpt-3.5-turbo'.

### Workflow Examples

#### On PR Opened
- `Feature A` branch opens a PR trying to merge into `main`
- Localize `Feature A` base language string files
- Opens a new PR `Feature A - Translations` based on `Feature A`
- Review and merge `Feature A - Translations` PR into `Feature A` branch
